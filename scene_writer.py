...
def _build_style_anchor(genre: str = "", style: str = "") -> str:
    """风格锚定注入 — 按题材选择对应的真人原文示例。
    基于 Bohr 2025 (Show+Tell) + Gao & Das 2024 (正面示例) + Tang 2025 (对比格式)。
    不同题材不混用——修仙不能套纯文学白描，科幻不能套武侠极简。
    数据来源: DSPy 训练集 6 条真人原文 + 联网验证的题材匹配分析。
    """
    # ── 题材 → 示例 映射 ──
    GENRE_ANCHORS = {
        # 修仙/玄幻/洪荒 — 忘语《凡人修仙传》白描 + 土豆《斗破》热血
        "修仙": [
            ("白描修仙 — 忘语《凡人修仙传》第一章",
             "二愣子睁大着双眼，直直望着茅草和烂泥糊成的黑屋顶，身上盖着的旧棉被，已呈深黄色，看不出原来的本来面目。离床大约半丈远的地方，是一堵黄泥糊成的土墙，因为时间过久，墙壁上裂开了几丝不起眼的细长口子，从这些裂纹中，隐隐约约的传来韩母唠唠叨叨的埋怨声，偶尔还掺杂着韩父抽旱烟杆的啪嗒啪嗒吸吮声。",
             "技法: 零比喻，全白描。用视觉(黑屋顶/旧棉被/土墙裂纹)、听觉(埋怨声/旱烟声)直接建立场景。一句话一层信息，不装饰。"),
        ],
        # 玄幻通用修仙的锚
        "玄幻": "修仙",
        "洪荒": "修仙",
        ...
        "科幻": [
            ("硬核科幻 — 刘慈欣《三体》",
             ...
             "技法: 用物理概念做喻体(引力/洪水)——这是解释性比喻，不删。区别于装饰性比喻(如'心像刀割')。"),
        ],
        # 悬疑/克苏鲁/灵异 — 乌贼《诡秘之主》
        "悬疑": [
            ("暗黑氛围 — 爱潜水的乌贼《诡秘之主》",
             "黑色的夜幕上，一轮赤红色的满月高高悬挂，撒下绯红色的流纱，寂静得恍若凝固。廷根市，码头区，一个脸色苍白、黑发散乱的年轻人从睡梦中惊醒。他的脸蓦地狰狞起来又蓦地平静下来，缓缓睁开半眯的充满血丝的双眼。",
             "技法: 用特定感官建立氛围(绯红满月/寂静)，动作推进(惊醒/睁眼)而非心理描写。"),
        ],
        "克苏鲁": "悬疑",
        "灵异": "悬疑",
        "盗墓": "悬疑",
    ...
    parts = ["\n【风格参照 — 同题材真人原文，学习叙事方法不抄袭句子】"]
    for title, text, technique in examples:
        parts.append(f"\n{title}:\n「{text}」\n{technique}")
    parts.append("\n请参照以上同题材叙事手法写作：用动作、感官、对话推进剧情，拒绝装饰性比喻。")
    return "\n".join(parts)
