...
**法则 3：改动前必须联网查真实来源，禁止为了交差瞎编**

网文写作技巧的唯一合法来源：万订作者访谈（白蘸糖/老鹰吃小鸡/会说话的肘子/爱潜水的乌贼等）、
起点编辑公开言论（山海/星河/青狐/无书等）、读者社区共识（龙空/NGA/知乎）、
行业数据分析（xs91/什么值得买/CSDN网文专栏）。

不允许凭感觉编造任何数据——结构节点、常见错误、金手指模式、开篇模板、爽点类型、钩子模板，
每一项都要能追溯到具体的人/帖子/数据。
每次修改 prompt 或 genre_knowledge 文件前，先搜索对应主题的真实讨论。
遇到不确定的内容，先搜索再写。搜不到就标注 `[待验证]`，绝不编造。
发现之前编造的内容立即修正（本次已验证修正：修仙结构节点、常见错误列表）。
...
**法则 6：代码和文档始终用中文**
项目全部中文。注释、docstring、变量名、commit message 都写中文。

**法则 7：区分人类作者经验和 AI 模型特性**
搜索到的网文创作建议，必须先判断"这是给人类作者的还是可以适用于 AI 的"。
人类心理效应（灵感透支/表达欲/沉没成本）对 AI 无效。
AI 特性相反：越详细的细纲输出越精准（detail in = detail out，2026 论文共识）。
