...
_register(GenreKnowledge(
    ...
    structure_nodes=[  # 2025趋势:升级流→讲修仙者故事,不换地图,5-6境界深耕
        {"position": "1.开篇压制", "ratio": "5%", "content": "主角被踩进泥里(退婚/被废/被辱)→结尾金手指初现", "emotion": "憋屈+好奇"},
        {"position": "2.转机试探", "ratio": "10%", "content": "金手指激活但不全开，解决眼前小危机", "emotion": "悬疑+期待"},
        {"position": "3.第一次打脸", "ratio": "15%", "content": "当众逆袭/升级/让反派吃惊，第一次小爽点", "emotion": "爽(初爽)"},
        {"position": "4.日常深耕", "ratio": "20%", "content": "市井奇遇(日常)+日行一善(收获)+斩妖除魔(长线)三条线交织", "emotion": "成长+期待"},
        {"position": "5.阻力升级", "ratio": "15%", "content": "反派加压/宗门斗争/资源危机，主角险境中挣扎", "emotion": "紧张+代入"},
        {"position": "6.大高潮", "ratio": "15%", "content": "当前阶段最强敌人决战，维度升级(解锁新能力而非纯数值)", "emotion": "爽(最高潮)"},
        {"position": "7.清算余韵", "ratio": "10%", "content": "反派受罚/规则重建/战利品收获，情感收束", "emotion": "宣泄+满足"},
        {"position": "8.新篇开启", "ratio": "10%", "content": "新危机/新目标露头，埋下更大伏笔，但不是换地图重启", "emotion": "期待"},
    ],
    ...
    common_mistakes=[  # 2025-2026社区共识+编辑反馈
        "境界太多(>8个)→套娃换地图→读者弃书。控制在5-6个大境界内深耕",
        "金手指没代价→开挂流水账。必须有代价:每用一次封印松动/消耗寿命/献祭记忆",
        "战力指数膨胀→战力写崩。用维度解锁(新能力)替代数值膨胀(10倍/级→1.5倍/级)",
        "开篇堆设定像说明书。设定必须在冲突中'漏'给读者,不能砸给读者",
        "跨大境界乱杀→战力体系崩溃。筑基不能随便打金丹",
    ],
    reader_drop_points=["30章没有明确主线", "境界太多看不下去", "换地图=换书"],
))
...
_register(GenreKnowledge(
    ...
    plot_cycle="农奴之子/铁匠学徒(身份)+被游方医生/隐士看中(金手指获得)→暗中修炼→资源匮乏+等级森严+丛林法则→不断获取资源突破阶层天花板→循环。换地图数据必掉，只能靠故事质量延缓。(白蘸糖巫师文案例:韩索尔，投靠舅舅没选上骑士仆从，被莫尔斯医生看中修炼《转生冥想法》)",
    structure_nodes=[
    ...
    common_mistakes=[  # 2025-2026社区共识+白蘸糖
        "世界观过度设定→开篇堆设定像说明书。必须在冲突中'漏'给读者",
        "金手指没代价→升级像喝水。每升一级有明确代价(资源耗尽/仇敌追杀/寿命消耗)",
        "换地图=战力重置+旧角色消失→读者弃书。用莲花写作法以一点为中心扩散,不硬切地图",
        "力量体系复杂到自己记不住→后期写崩。控制在读者能记住的范围内",
    ],
    reader_drop_points=["开篇设定堆砌读不下去", "换地图后像换了一本书", "主角成长无代价太无聊"],
))
...
_register(GenreKnowledge(
    ...
    plot_cycle="落魄身份(程序员失业>快递员失业,身份决定代入感)+百倍暴击/系统(金手指必须快节奏)→快速解决经济困境(第一循环)→财富+地位(第二循环)→情感线(核心循环,都市文上限靠情感线,忌讳纯写赚钱事业)。禁忌关系通过身份推进制造期待感。(白蘸糖:都市文最永恒话题=财富+地位+女人)",
    structure_nodes=[
...
_register(GenreKnowledge(
    ...
    common_mistakes=[  # 2025重生文社区共识
        "开篇进剧情太慢→前3章还在写前世。删掉累赘,直逼主题",
        "主角太懦弱→重生后还是废物心态。读者等不到反击就弃了",
        "复仇代价不对等→前世痛苦一笔带过,今生折磨写几十万字",
        "50万字后崩盘→依赖信息差爽点,读者追读率平均下降42%",
        "金手指出场太晚→第二章还没亮金手指=暴死",
    ],
))
...
_register(GenreKnowledge(
    ...
    common_mistakes=[  # 2025-2026无限流/规则怪谈社区共识
        "规则写了几十条跟剧情无关→纯水字数。规则必须为剧情服务",
        "金手指与副本难度失衡→要么无敌要么全程挨打。胜率控制在10-30%黄金区间",
        "副本间无主线串联→变成短篇合集。每个副本结尾留可追溯的主线线索",
        "死亡惩罚只有暴毙→比死人更高级:同伴遗忘记忆/牺牲NPC/信任崩塌",
        "开篇堆世界观说明书→主神空间设定必须在冲突中随主角行动逐步揭露",
    ],
))
