...
async def generate_chapter_outline(request: Request):
    ...
    system_prompt = f"""你是网文细纲设计师。为第{chap_num}章输出结构化施工图。

小说：{novel_title} | 类型：{genre}
世界观参考：{world_ctx[:1500]}{planning_context}

【输出铁律 — 违反即失败】
- 只用动作动词和具体名词写要点(如「林渊挖到神格→赵虎抢夺→林渊反杀」)
- 禁止叙事句(如「主角感到一股前所未有的力量」=失败)
- 禁止AI模板词: 一股/前所未有/不可名状/仿佛/如同/不是A而是B
- 每行要点≤30字，电报风格。你是在列施工图纸，不是在写小说

【输出格式 — 严格按此字段填写】

## 第{chap_num}章: [章节标题]

### 核心节拍（2-3个动作句，电报风格，≤30字/条）
- [主角做了什么，遇到什么阻碍]
