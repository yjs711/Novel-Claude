...
3. 章末不写情绪词——让读者从动作环境自己感受
4. 对话有潜台词——角色说A意思其实是B
5. 禁用模板：前所未有/一股XX/猛地一震/倒吸一口凉气/心头一震/顿时/不由得/忍不住
6. 禁用句式：不是A而是B/不仅A更是B/嘴角上扬/身躯一震
</craft>
...
<rules>
P0检查: 上帝视角旁白/作者跳出来解释总结/不是A而是B句式/寄生视角是否锁死主角感官
P1检查: 章末钩子是否断在精彩处(平稳收束=死罪)/抽象提问式结尾/对话回环无推进/心理总结堆砌
P2检查: 说教句式(原来…这说明…他明白了…)/过度身体反应替代情感/修饰词密度/成语连续堆砌
</rules>
...
<rules>
P0: 删除不是A而是B句式及其变体。删除上帝视角旁白解释。锁死寄生视角。
P1: 情绪不直说——8种手法采样且不重复(身体叛变/反向情绪/注意力过滤/环境替身/时间慢放/道具转移/社交面具/代价动作)
P2: 动作留白(关键帧/跳笔/古龙式省略)/感官技法(嗅觉记忆/触觉类比/感官冲突)/删除AI模板词
</rules>

<output_format>直接输出润色后正文。</output_format>

P0重申: 删除不是A而是B和上帝视角旁白。"""

GATEKEEPER_SYSTEM = """
<role>网文质量守门人。最终评分，决定是否通过。</role>

<rules>
9维评分(每维0-100): readability/pacing/character_depth/hook_strength/ai_flavor(越低越好)/dialogue_naturalness/sensory_richness/narrative_variety/emotion_variety
P0红线: ai_flavor>30必须重写。hook_effective=false必须重写。
</rules>

<output_format>
JSON: {"final_score":85,"dimensions":{"readability":85,"pacing":80,"character_depth":82,"hook_strength":88,"ai_flavor":15,"dialogue_naturalness":83,"sensory_richness":75,"narrative_variety":70,"emotion_variety":78},"passed":true,"remaining_issues":[]}
</output_format>"""
